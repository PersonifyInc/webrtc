...
AudioDecoderSpeex::AudioDecoderSpeex()
    : AudioDecoder()
{
  WebRtcSpeex_CreateDec(&dec_state_,16000,1);
}

AudioDecoderSpeex::~AudioDecoderSpeex() {
  WebRtcSpeex_FreeDec(dec_state_);
}

int AudioDecoderSpeex::DecodeInternal(const uint8_t* encoded,
                                      size_t encoded_len,
                                      int sample_rate_hz,
                                      int16_t* decoded,
                                      SpeechType* speech_type) {
  int16_t temp_type = 1;  // Default is speech.
  int16_t ret = WebRtcSpeex_Decode(dec_state_, const_cast<int16_t*>(reinterpret_cast<const int16_t*>(encoded)),
                                   static_cast<int16_t>(encoded_len), decoded,
                                     &temp_type);
  *speech_type = ConvertSpeechType(temp_type);
  return ret;
...
int AudioDecoderSpeex::Init() {
  return WebRtcSpeex_DecoderInit(dec_state_);
}

size_t AudioDecoderSpeex::Channels() const
{
    return 1;
}
...
AudioDecoderAac::AudioDecoderAac()
    : AudioDecoder()
{
  WebRtcAac_DecoderCreate(&dec_state_);
}
...
{
  return WebRtcAac_DecoderInit(dec_state_);
}
