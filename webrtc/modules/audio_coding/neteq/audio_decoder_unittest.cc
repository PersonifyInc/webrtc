...
#ifdef WEBRTC_CODEC_SPEEX
  EXPECT_EQ(8000, CodecSampleRateHz(kDecoderSPEEX_8));
  EXPECT_EQ(16000, CodecSampleRateHz(kDecoderSPEEX_16));
#else
  EXPECT_EQ(-1, CodecSampleRateHz(kDecoderSPEEX_8));
  EXPECT_EQ(-1, CodecSampleRateHz(kDecoderSPEEX_16));
#endif
...
TEST(AudioDecoder, CodecSupported) {
  ...
  EXPECT_TRUE(CodecSupported(kDecoderOpus_2ch));
#ifdef WEBRTC_CODEC_SPEEX
  EXPECT_TRUE(CodecSupported(kDecoderSPEEX_8));
  EXPECT_TRUE(CodecSupported(kDecoderSPEEX_16));
#else
  EXPECT_FALSE(CodecSupported(kDecoderSPEEX_8));
  EXPECT_FALSE(CodecSupported(kDecoderSPEEX_16));
#endif
}
