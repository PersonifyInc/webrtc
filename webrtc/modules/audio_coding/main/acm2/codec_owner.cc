...
bool IsAAC(const CodecInst& codec) {
  return
#ifdef WEBRTC_CODEC_AAC
      !STR_CASE_CMP(codec.plname, "aac") ||
#endif
      false;
}

bool IsSpeex(const CodecInst& codec) {
  return
#ifdef WEBRTC_CODEC_SPEEX
      !STR_CASE_CMP(codec.plname, "speex") ||
#endif
      false;
}
...
void CreateSpeechEncoder(
  ...
  } else if (IsAAC(speech_inst)) {
#ifdef WEBRTC_CODEC_AAC
    speech_encoder->reset(new AudioEncoderMutableAac(speech_inst));
#endif
 } else if (IsSpeex(speech_inst)) {
    speech_encoder->reset(new AudioEncoderMutableSpeex(speech_inst));
  } else {
    FATAL();
  }
